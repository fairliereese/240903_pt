...
def fmt_gtf(ifile, ofile):
    ...
    df = df.loc[df.Feature.isin(['exon'])]
    ...
    # make new gene entries for everything
    l1 = len(df.transcript_id.unique().tolist())
    # make gene entry
    t_df = make_hier_entry(df, how='t')
    t_df['Source'] = 'novel_gene'
    t_df['Frame'] = '.'
    t_df['Score'] = '.'
    l2 = len(t_df.loc[t_df.Feature=='transcript'].index)
    assert l1 == l2

    # concat them and then sort gtf
    df = pd.concat([df, g_df, t_df], axis=0)
